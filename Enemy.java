...
public class Enemy extends Entity{
    ...
    private BufferedImage getNewImage(int alphaValue){
        Color enemyBaseColor = new Color(this.baseColor.getRed(),this.baseColor.getGreen(),
        this.baseColor.getBlue(),visibility);
        return createEnemyImage(enemyBaseColor);
    }

    public void makeAiChoice(Drone primaryDrone){
        if (this.getStepDistance() != 0){
            this.move(new int[]{this.getX(),this.getY()},primaryDrone);
        }
    }

    private void move(int[] initialCoords, Drone primaryDrone){
        int xDiff = (Constants.WIDTH/2)-initialCoords[0];
        int yDiff = (Constants.HEIGHT/2)-initialCoords[1];
        int newX;
        int newY;
        int directionX; 
        int directionY; 

        if ((Constants.WIDTH/2)> initialCoords[0]){
            directionX = 1; 
        } else if ((Constants.WIDTH/2) < initialCoords[0]){
            directionX = -1;       
        } else {
            directionX = 0; 
        }

        if ((Constants.HEIGHT/2) > initialCoords[1]){
            directionY = 1; 
        } else if ((Constants.HEIGHT/2) < initialCoords[1]){
            directionY = -1; 
        } else {
            directionY = 0;
        }
    ...
    //set transparency value / alpha value of its color 
